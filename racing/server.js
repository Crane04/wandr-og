...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'host-create') {
      ...
      if (!rooms[code]) rooms[code] = { hosts: new Set(), controllers: new Set() };
      rooms[code].hosts.add(ws);
      ws.role = 'host';
      ws.room = code;
      ws.mode = msg.mode === 'ai' ? 'ai' : 'two';
    ...
    if (msg.type === 'input' && ws.role === 'controller') {
      ...
      if (room) {
        for (const host of room.hosts) {
          if (host.mode === 'ai' && ws.playerNum !== 1) continue;
...
server.listen(PORT, () => {
  const ip = localIp();
  console.log('Racing server running:');
  console.log(`  Laptop (game):      http://${ip}:${PORT}/`);
  console.log(`  Phone (controller): http://${ip}:${PORT}/controller.html`);
});
